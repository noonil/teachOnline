...
@interface HSLectureHsDetailViewModel ()

@property (strong, readwrite, nonatomic) NSArray *stageList;

@end

@implementation HSLectureHsDetailViewModel
...
//    }
    ...
    if (self.lectureModel.lectureID.length > 0) {
        [parameter setObject:self.lectureModel.lectureID forKey:@"courseId"];
    }
    
//    /HsMobile/com/course/stage/selectCourseStageOnlyVideo
    __weak typeof(self) weakSelf = self;
    //课程阶段目录列表接口
    [[EPHttpClient sharedClient] POST:@"com/courseStage/selectCourseStageOnlyVideo" parameters:parameter success:^(AFHTTPRequestOperation *operation, id responseObject) {
        
        NSDictionary *stageDicts = responseObject[@"data"];
        NSArray *stageArr = stageDicts[@"stageList"];
        ...
        if (IsArrayClass(stageArr)) {
            ...
            for (NSMutableDictionary *stageDict in stageArr) {

                HSLectureStageModel *stageItem = [[HSLectureStageModel alloc] initWithDict:stageDict];
                            [stageItem setValue:stageDicts[@"classhourCount"] forKey:@"classhourCount"];
                            [stageItem setValue:stageDicts[@"homeworkCount"] forKey:@"homeworkCount"];
