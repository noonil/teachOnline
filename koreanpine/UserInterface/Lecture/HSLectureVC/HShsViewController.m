...
@implementation HShsViewController
- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"hsRefreshNotification" object:nil];
}
- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    [self initView];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(receiveNotify:) name:@"hsRefreshNotification" object:nil];
}
-(void)receiveNotify:(NSNotification *)notify{
    [self.lectureHsViewModel fetchLatest];
    WS(weakSelf);
    self.lectureHsViewModel.reloadBlock = ^() {
       [weakSelf.yhCollectionHsView reloadData];
    };
...
//    [self.view addSubview:self.yhCollectionView];
        [self.view addSubview:self.yhCollectionHsView];
...
//    
            ...
            [self.yhCollectionHsView addLoadMoreActionHandler:^{
                [weakSelf.lectureHsViewModel fetchMore];
            }];
...
//        
//        _yhCollectionView = [[HSCollectionView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.bounds), CGRectGetHeight(self.view.bounds)-49.f) collectionViewLayout:flowLayout];
...
- (HSCollectionView *)yhCollectionHsView
{
    if (!_yhCollectionHsView) {
        UICollectionViewFlowLayout *flowLayout = [[UICollectionViewFlowLayout alloc] init];
//        [flowLayout setDelegate:self];
        [flowLayout setSectionInset:UIEdgeInsetsMake(10, 5*kScreenPointScale, 10, 5*kScreenPointScale)];
        [flowLayout setMinimumInteritemSpacing:0*kScreenPointScale];
        ...
        _yhCollectionHsView = [[HSCollectionView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.bounds), CGRectGetHeight(self.view.bounds)-49.f-55) collectionViewLayout:flowLayout];
        [_yhCollectionHsView setAlwaysBounceVertical:YES];
        [_yhCollectionHsView setAutoresizingMask:UIViewAutoresizingFlexibleHeight|UIViewAutoresizingFlexibleWidth];
        [_yhCollectionHsView setBackgroundColor:RGBCOLOR(235, 235, 235)];
        [_yhCollectionHsView setDelegate:self];
        [_yhCollectionHsView setDataSource:self];
        
        [_yhCollectionHsView registerClass:[HSLectureHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"HSLectureHeaderView"];
        [_yhCollectionHsView registerClass:[HSLectureTopCell class] forCellWithReuseIdentifier:@"HSLectureTopCell"];
        [_yhCollectionHsView registerClass:[HSLectureItemCell class] forCellWithReuseIdentifier:@"HSLectureItemCell"];
    }
    return _yhCollectionHsView;
}
...
{
    HSLectureModel *lectureModel = (HSLectureModel *)[self.lectureHsViewModel contentAtIndex:0];
    lectureModel.isHs = YES;
    if (!lectureModel) {
        [[Hud defaultInstance] showMessage:@"暂无有效数据"];
        return;
    }
    HSLectureDetailVC *nextVC = [[HSLectureDetailVC alloc] initWithNibName:@"HSLectureDetailVC" bundle:nil lectureModel:lectureModel isHs:YES ClassID:nil];
    [self.navigationController pushViewController:nextVC animated:YES hideBottomTabBar:YES];
}
...
{
    NSInteger count = [self.lectureHsViewModel contentCount]-1;
    return count;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    itemCell.isHide = YES;
    HSLectureModel *lectureModel = (HSLectureModel *)[self.lectureHsViewModel contentAtIndex:indexPath.row+1];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    headerView.isHide = YES;
    HSLectureModel *lectureModel = (HSLectureModel *)[self.lectureHsViewModel contentAtIndex:indexPath.row];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    lectureModel.isHs = YES;
    HSLectureDetailVC *nextVC = [[HSLectureDetailVC alloc] initWithNibName:@"HSLectureDetailVC" bundle:nil lectureModel:lectureModel isHs:YES ClassID:nil];
    [nextVC addViewedCourse:lectureModel.lectureID isHs:YES];
//    [self initView];
    [self.navigationController pushViewController:nextVC animated:YES hideBottomTabBar:YES];
...
{
    return CGSizeMake(155.0f*kScreenPointScale, 120*kScreenPointScale);
}
...
//    
        ...
        [self.yhCollectionHsView.footer setHidden:!self.lectureHsViewModel.hasMoreItems];
