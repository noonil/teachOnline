...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        // 初始化文字
        [self setTitle:NSLocalizedString(@"下拉可以刷新", nil) forState:MJRefreshHeaderStateIdle];
        [self setTitle:NSLocalizedString(@"松开立即刷新", nil) forState:MJRefreshHeaderStatePulling];
        [self setTitle:NSLocalizedString(@"正在刷新内容中...", nil) forState:MJRefreshHeaderStateRefreshing];
    }
    return self;
}
...
- (void)setUpdatedTime:(NSDate *)updatedTime
{
    ...
    if (updatedTime) {
        ...
        // 3.显示日期
        self.updatedTimeLabel.text = [NSLocalizedString(@"最后更新：", nil) stringByAppendingString:[NSString stringWithFormat:@"%@", time]];
    } else {
        self.updatedTimeLabel.text = NSLocalizedString(@"最后更新：无记录", nil);
    }
}
