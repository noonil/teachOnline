...
- (instancetype)initWithURL:(NSURL *)url
               ...
               documentType:(TCBlobDownloadDocumentType)documentType
...
{
    self = [self initWithURL:url downloadPath:pathToDL documentType:documentType delegate:nil];
    if (self) {
...
- (BOOL)isFinished
{
    [self sendChangeStateNotification];
...
- (void) sendChangeStateNotification {
    [[NSNotificationCenter defaultCenter] postNotificationName:kChangeDownloadItemStateNotification object:self];
}
...
- (NSString *)fileName
{
    NSString *defaultName = [Common MD5FromString:[self.downloadURL absoluteString]];
    switch (self.documentType) {
        case TCBlobDownloadDocumentTypeVideo:
            defaultName = [NSString stringWithFormat:@"%@.mp4",defaultName];
            break;
        case TCBlobDownloadDocumentTypePDF:
            defaultName = [NSString stringWithFormat:@"%@.pdf",defaultName];
            break;
        default:
            break;
    }
    return _fileName ? _fileName : defaultName;//[[NSURL URLWithString:[self.downloadURL absoluteString]] lastPathComponent];
}
